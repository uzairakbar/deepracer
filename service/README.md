...
simulation served to the `deepracer` client over ZMQ.
...
```bash
service/scripts/init_workspace.sh     # move upstream@pin + patches to workspace/
cd service/workspace                  # normal git repo; edit, commit, test here
# ... make changes, git commit ...
service/scripts/export_patches.sh     # write commits back to ../patches/
git add service/patches && git commit # commit patches to this monorepo
```
...
```bash
service/scripts/bump_upstream.sh <new-upstream-tag>
# resolve any 3-way conflicts in workspace/, rebuild + validate, then:
service/scripts/export_patches.sh
git add service/upstream service/patches
