...
from deepracer.utils import terminated_check, truncated_check
from deepracer.zmq_client import DeepracerClientZMQ
...
def DUMMY_ACTION_CONTINUOUS() -> np.ndarray:
...
class DeepracerGymAdapter:
    def __init__(self, action_space_type: str, host: str = HOST, port: int = PORT):
        if action_space_type == "discrete":
            ...
            raise ValueError(
                "Action space can only be discrete or continuous. "
                f"Got {action_space_type} instead."
            )
    ...
    def env_reset(self):
        ...
        step = self.response["info"]["reward_params"]["steps"]
        while step != 1:
            self.response = self._send_action(self.dummy_action())
            step = self.response["info"]["reward_params"]["steps"]
    ...
    def _parse_response(response: dict):
        ...
        observation = {
            sensor: (
                measurement.transpose(-1, 0, 1) if "CAMERA" in sensor else measurement
            )
