...
def validate_action_space_config(config: dict, action_space_type: str):
    ...
    if action_space_type == "discrete":
        assert isinstance(config["action_space"], list), (
            "action_space_type is discrete but action_space is not a list."
        )

        for action in config["action_space"]:
            assert isinstance(action, dict), (
                "All actions should be defined as dictionaries in action_space."
            )

            assert all(
                (key in action and isinstance(action[key], (int, float)))
                for key in ("steering_angle", "speed")
            ), "steering_angle or speed incorrectly defined for action in action_space."
    elif action_space_type == "continuous":
        assert isinstance(config["action_space"], dict), (
            "action_space_type is continuous but action_space is not a dictionary."
        )

        assert all(
            (
                key in config["action_space"]
                and isinstance(config["action_space"][key], dict)
            )
            for key in ("steering_angle", "speed")
        ), "steering_angle or speed incorrectly defined for action in action_space."
        ...
        for action, bounds in config["action_space"].items():
            assert all(
                (key in bounds and isinstance(bounds[key], (int, float)))
                for key in ("low", "high")
            ), f"Bounds incorrectly defined for {action} in action_space."

            assert bounds["low"] < bounds["high"], (
                f"Lower bound should be lower than upper bound "
                f"for {action} in action_space."
            )
    else:
        raise ValueError("action_space_type can only be continuous or discrete.")
...
def action_space_type(config: dict):
    if "action_space_type" in config:
        if config["action_space_type"] not in ("discrete", "continuous"):
            raise ValueError("Incorrectly defined action_space_type in config file.")
        ...
        if isinstance(config["action_space"], list):
            ...
            raise ValueError("Incorrectly defined action_space in config file.")
    return space_type
...
def make_action_space(config=None):
    ...
    from deepracer.defaults import resolve_agent_config

    config = resolve_agent_config(config)

    assert "action_space" in config, "Action space not defined in agent config."

    try:
        space_type = action_space_type(config)
    except Exception as e:
        logger.error("Incorrectly defined action_space in agent config.")
        raise e
    ...
    if space_type == "discrete":
        ...
        action_space = spaces.Discrete(size)
    elif space_type == "continuous":
        action_space = spaces.Box(low=-1, high=1, shape=(2,), dtype=np.float64)
    else:
        raise ValueError(
            "Space type can only be discrete or continuous for actions. "
            f"Got {space_type} instead."
        )
...
def make_observation_space(config=None):
    ...
    for sensor in sensors:
        assert (sensor in SENSOR_SPACE) and (SENSOR_SPACE[sensor] is not None), (
...
def string_to_port(string):
    ...
    hash_int = int.from_bytes(
        hash_bytes[:4],
        byteorder="big",  # Only first 4 bytes
    )
