...
def test_start_alive_logs_materialize_stop(fake_image, docker_backend, cleanup_managed):
    ...
    handle = mgr.acquire(
        agent_config=AGENT, track_config=TRACK, image=fake_image, **SMALL
    )
    try:
        ...
        got_agent = json.loads(
            _exec(handle, docker_backend, "cat /configs/agent_params.json")
        )
        assert got_agent == AGENT
        got_track = _exec(
            handle, docker_backend, "cat /configs/environment_params.yaml"
        )
...
def test_concurrent_distinct_configs_and_cap(
    fake_image, docker_backend, cleanup_managed
):
    ...
    try:
        ...
        with pytest.raises(RuntimeError, match="slots are in use"):
            mgr.acquire(
                agent_config=AGENT,
                track_config={"WORLD_NAME": "Monaco"},
                image=fake_image,
                **SMALL,
            )
    finally:
...
def test_keep_warm_reuses_container(fake_image, docker_backend, cleanup_managed):
    ...
    h1 = mgr.acquire(agent_config=AGENT, track_config=TRACK, image=fake_image, **SMALL)
    ...
    h2 = mgr.acquire(agent_config=AGENT, track_config=TRACK, image=fake_image, **SMALL)
    try:
...
def test_keep_warm_different_config_starts_fresh(
    fake_image, docker_backend, cleanup_managed
):
    mgr = _mgr(docker_backend, "pytest-warm2")
    h1 = mgr.acquire(
        agent_config=AGENT,
        track_config={"WORLD_NAME": "Austin"},
        image=fake_image,
        **SMALL,
    )
    mgr.release(h1, keep_warm=True)
...
def test_two_managers_same_config_distinct_ports(
    fake_image, docker_backend, cleanup_managed
):
...
def test_readiness_surfaces_fatal_on_crash(
    fake_image, docker_backend, cleanup_managed, monkeypatch
):
