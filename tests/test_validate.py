...
from deepracer.service.spec import SimSpec, spec_to_env
...
def test_bad_action_space_rejected():
    agent = dict(
        DISCRETE_AGENT, action_space=[{"speed": 0.6}]
    )  # missing steering_angle
...
def test_eval_world_validated():
    with pytest.raises(ValueError, match="Unknown WORLD_NAME"):
        validate_configs(
            DISCRETE_AGENT, GOOD_TRACK, world_name="nope_zzz", evaluation=True
        )


def test_spec_to_env_has_expected_keys_and_roundtrips():
    spec = SimSpec(
        identity=make_identity("alice", 0),
        agent_config=DISCRETE_AGENT,
