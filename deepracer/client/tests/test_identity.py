...
from deepracer_gym.service.identity import (
    make_identity, string_to_port, port_is_free, free_env_id, fingerprint,
    PORT_LO, PORT_HI, DISPLAY_BASE,
)
...
def test_free_env_id_skips_taken_and_busy(monkeypatch):
    ...
    import deepracer_gym.service.identity as idm
    busy_ports = {make_identity('u', 0).port, make_identity('u', 1).port}
    monkeypatch.setattr(idm, 'port_is_free', lambda p, host='127.0.0.1': p not in busy_ports)
