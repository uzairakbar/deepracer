...
**Reaping:** A cached simulator is only reused within the same Python process. When you are completely done, or if you want to reclaim resources, stop managed simulators:
...
**Capping:** By default, a process can manage up to `DEEPRACER_MAX_ENVS=4` simulators at the
...
Unfortunately, `AsyncVectorEnv` is not yet supported by the current version of `deepracer_gym`.
